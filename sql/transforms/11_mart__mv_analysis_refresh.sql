...
DROP MATERIALIZED VIEW IF EXISTS mart.mv_analysis_state_month;
CREATE MATERIALIZED VIEW mart.mv_analysis_state_month AS
SELECT
  s.state_code,
  m.month_date,
  m.ym,
  ...
  CASE
    WHEN fr.sales_mkwh IS NULL OR fr.sales_mkwh = 0 THEN NULL
    ELSE (COALESCE(dc.total_mw, 0) / fr.sales_mkwh)
  END AS dc_mw_per_res_mkwh
