...
--
-- Cost proxy design (transparent + regression-friendly):
--   - Extract "best representative" cost per group per state-month by choosing the row
--     with the highest generation within that group (avoids double counting when both
--     component rows and "all X" rows exist).
--   - Compute global z-scores for coal and natural gas costs across all state-months.
--   - fuel_cost_proxy = share_gas*z_gas + share_coal*z_coal
--   This yields a dimensionless index capturing "fossil-heavy + expensive fossil fuel"
--   conditions. Python can later replace/extend this with richer proxies if desired.
-- =============================================================================

BEGIN;

-- -------------------------------------------------------------------------
-- 1) stage.ops_state_month_fuel (cleaned mirror of raw ops)
-- -------------------------------------------------------------------------
...
--    Use ops_sector_id='1' ("Electric Utility") for a stable, utility-centric signal.
-- -------------------------------------------------------------------------
...
WITH base AS (
  SELECT
    ...
    (generation_thousand_mwh * 1000.0) AS generation_mwh, -- convert to MWh
    cost,
    cost_units
...
best_in_group AS (
  -- Pick the single "best representative" row per (state, month, fuel_group)
  -- using maximum generation to avoid double counting (e.g., "all coal products" vs subtypes).
  ...
  SELECT
    *,
    CASE WHEN generation_mwh_total IS NULL OR generation_mwh_total = 0 THEN NULL
         ELSE generation_mwh_coal    / generation_mwh_total END AS share_coal,
    CASE WHEN generation_mwh_total IS NULL OR generation_mwh_total = 0 THEN NULL
         ELSE generation_mwh_gas     / generation_mwh_total END AS share_gas,
    CASE WHEN generation_mwh_total IS NULL OR generation_mwh_total = 0 THEN NULL
         ELSE generation_mwh_nuclear / generation_mwh_total END AS share_nuclear,
    CASE WHEN generation_mwh_total IS NULL OR generation_mwh_total = 0 THEN NULL
         ELSE generation_mwh_renew   / generation_mwh_total END AS share_renew
  FROM totals
),
zscored AS (
  SELECT
