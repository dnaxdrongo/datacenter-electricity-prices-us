-- Materialized view is nice for speed + reproducibility
...
LEFT JOIN mart.fact_retail_price frp
  ON frp.state_sk = s.state_sk
 AND frp.month_sk = m.month_sk
LEFT JOIN mart.dim_retail_sector rs
  ON rs.sector_sk = frp.sector_sk
...
CREATE INDEX IF NOT EXISTS ix_mv_analysis_state_month ON mart.mv_analysis_state_month(state_code, month_date);
